...
正式路线为 `v1.0 → v1.1 → v1.2 → v1.3 → v1.4 → v1.5 → v1.6 → v1.7 → v1.8 → v1.9 → v1.9.1 → v1.10`。v1.0 是已关闭的 governance baseline，v1.1 至 v1.9.1 均已完成并关闭；v1.9.1 只做 classifier `other` boundary 的最小 prompt clarification、focused offline regression 与 focused real-provider validation，不改变 production semantic architecture。v1.9 的第一次 scheduled Gen2 production acceptance 为 PASS WITH ACCEPTED DEGRADATION，full offline release gate 与 category presentation corrective 也均 PASS，后者仅调整 reader-facing section 顺序，canonical Selector/Event/Brief order 不变。下一步继续观察 Generation 2 production stability，再开始 v1.10 Legacy Retirement READ-ONLY dependency audit；v1.9 仍禁止 automatic Generation 1 semantic fallback/rollback，v1.10 implementation 与 legacy cleanup 尚未开始。完整 milestone scope 与治理规则见 [`docs/DECISIONS.md`](docs/DECISIONS.md) 和 [`docs/BACKLOG.md`](docs/BACKLOG.md)。Market 不属于 v1.x core，Holdings 不进入 v1.x。
