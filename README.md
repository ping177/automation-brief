...
当前版本为 `v0.6.2 — AI Curator Shadow Evaluation（Phase 4B live policy frozen）`。v0.6.0-alpha 的 AI Curator shadow foundation、Phase 3B fixture gate 和 Phase 4B provider-facing projection 作为 shadow-only 基础能力保留；生产 daily digest / `market_brief` 仍不接 DeepSeek、Tavily、其他真实 AI provider，也不依赖任何付费搜索 API。Phase 4 live shadow 必须显式使用 `--input-mode phase4_live`；第一次 real RSS + DeepSeek shadow 已保持 shadow-only，输入和 transport 正常，但 real output 因 response collection invariant 失败而 fail closed，不切换生产路径。
