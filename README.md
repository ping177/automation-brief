...
当前 `v1.9.1 — Classifier “other” Boundary Correction` 已 COMPLETED / CLOSED：最小 category-boundary prompt clarification、focused offline regression、validation-only `--classifier-only` compact-fixture runner 与 3/3 focused real-provider validation 均通过；不改变 production routing、LaunchAgent、delivery、canonical taxonomy、Selector、Writer 或 v1.10 时序。partial banner 未在本 corrective 中修改，作为独立 follow-up 保留。
当前 `v1.9.2 — Source Timezone Normalization` 已 COMPLETED / CLOSED：仅为显式声明 source timezone 的 Gen2 source-scoped naive timestamp 提供 IANA timezone localization；Investing.com 中文财经声明 `UTC`，未声明 timezone 的 source 继续 fail closed。offline ingest validation 与 controlled live RSS validation 均 PASS；用户执行的 source-only live run 对 10/10 entries 完成 normalization 且 `failure_codes=[]`。不修改 canonical datetime contract、production routing、LaunchAgent、delivery、partial banner 或 semantic stages。v1.10 尚未开始。
当前 `v1.9.3 — Classifier Boundary Correction` 已 COMPLETED / CLOSED：2026-08-31 production 中韩国总统李在明内阁改组/部长提名与中国中央三部门全国性商品住房销售制度改革曾由 provider 合法返回 `other`，根因是现有 prompt 缺少 `geopolitics` / `china_policy` 的正向 category boundary，而非 parser 或 overlay；本 corrective 仅补充 prompt boundary。v1.9.3 focused fixture 保留 v1.9.1 五个 case 并新增两个 case；30/30 offline smoke PASS，用户连续 3 次 real-provider validation 均 `classifier_stage_status=succeeded`、`technical_failures=[]`、7/7 match。首次 `invalid_input` invocation 仅因 shell 未加载 `.env.local` / process-env credential，不是产品、fixture 或 classifier defect，也不计为 quality failure。clustering 保持现有 frozen policy，2026-08-31 disaster overmerge 与一次 VentureBeat `transport_failed` 仅作 observation / accepted transient degradation，不构成当前 blocker；v1.10 尚未开始，下一步继续观察 Generation 2 scheduled production stability。
...
v0.7.3 七天真实使用验证已 CLOSED，作为 Generation 1 baseline 保留；产品 review 支持停止继续 patch Generation 1 核心新闻架构。v1.0 已完成 Architecture Freeze、READ-ONLY Dependency Audit、Core Data Contract Freeze 和 Runtime / Failure Contract Freeze，v1.x implementation roadmap 已冻结，迁移路线为 preserve mature infrastructure + rewrite news core。v1.1 至 v1.9.3 均已完成并关闭；v1.9.2 post-v1.9.1 source corrective 已通过 offline ingest 与 controlled live RSS validation，v1.9.3 classifier boundary corrective 已通过 30/30 offline 与 3/3 focused real-provider validation。v1.8 的 manual rolling-24h real run、完整 Gen2 pipeline 与用户 reader-facing 人工验收均 PASS，v1.9 的第一次 scheduled production acceptance 为 PASS WITH ACCEPTED DEGRADATION。这些 milestone 均未删除 legacy。Generation 2 已成为 active production route，Generation 1 继续保留为 explicit human-approved rollback route，v1.10 尚未开始。架构职责见 [`docs/EVENT_DRIVEN_MORNING_BRIEF_ARCHITECTURE.md`](docs/EVENT_DRIVEN_MORNING_BRIEF_ARCHITECTURE.md)，canonical object contract 见 [`docs/EVENT_DRIVEN_MORNING_BRIEF_DATA_CONTRACT.md`](docs/EVENT_DRIVEN_MORNING_BRIEF_DATA_CONTRACT.md)，runtime / failure contract 见 [`docs/EVENT_DRIVEN_MORNING_BRIEF_RUNTIME_CONTRACT.md`](docs/EVENT_DRIVEN_MORNING_BRIEF_RUNTIME_CONTRACT.md)。
...
输出只报告 run outcome 与 canonical artifact directory；主 artifact 只写入 `<data-root>/runs/event-driven-morning-brief/<run_id>/`。该命令不投递、不写 canonical `reports/`，也未加入任何 schedule。v1.8 closeout 已由用户确认：一次有效 rolling-24h real run 与 reader-facing Brief 人工验收通过；Selector/Writer 的已知局部 provider variation 均按既有 failure semantics 正确隔离。Event Clustering 不追求理论上的 100% 同事件识别，只有长期真实使用中反复出现明显 duplicate、overmerge 或不可接受 split 时才重新打开 clustering。v1.9.3 classifier boundary corrective 已完成并关闭，下一步继续观察 Generation 2 scheduled production stability；v1.10 尚未开始。
...
正式路线为 `v1.0 → v1.1 → v1.2 → v1.3 → v1.4 → v1.5 → v1.6 → v1.7 → v1.8 → v1.9 → v1.9.1 → v1.9.2 → v1.9.3 → v1.10`。v1.0 是已关闭的 governance baseline，v1.1 至 v1.9.3 均已完成并关闭；v1.9.1 只做 classifier `other` boundary 的最小 prompt clarification、focused offline regression 与 focused real-provider validation，不改变 production semantic architecture。v1.9.2 只做 source-scoped timezone metadata normalization，offline 与 controlled live RSS validation 均 PASS。v1.9.3 只补充 `geopolitics` / `china_policy` category boundary，offline 30/30 与 focused real-provider validation 3/3 均 PASS。v1.9 的第一次 scheduled Gen2 production acceptance 为 PASS WITH ACCEPTED DEGRADATION，full offline release gate 与 category presentation corrective 也均 PASS，后者仅调整 reader-facing section 顺序，canonical Selector/Event/Brief order 不变。下一步观察 Generation 2 production stability，再开始 v1.10 Legacy Retirement READ-ONLY dependency audit；v1.9.3 不改变 automatic Generation 1 semantic fallback/rollback policy，v1.10 implementation 与 legacy cleanup 尚未开始。完整 milestone scope 与治理规则见 [`docs/DECISIONS.md`](docs/DECISIONS.md) 和 [`docs/BACKLOG.md`](docs/BACKLOG.md)。Market 不属于 v1.x core，Holdings 不进入 v1.x。
...
Slice 2–5 的 Generation 2 route 仍必须显式传入；该 route 使用 Asia/Shanghai report date 贯穿 adapter、publisher 与 Bark，Slice 4 full offline gate 已通过，Slice 5 installed activation 已应用：
...
→ 08:00 launchd 自动运行
→ scripts/run_daily_digest.sh generation_2
→ run_generation_2_production.py 生成并发布 canonical Morning Brief
→ publish_mobile_digest.py --report-type overnight_brief --report-date YYYY-MM-DD 同步到 Obsidian iCloud
→ send_bark_notification.py --report-type overnight_brief --report-date YYYY-MM-DD 发送 Bark 推送
→ 点击 Bark 通知直达 iPhone Obsidian 当天 Morning Brief
```

自动唤醒与 launchd 08:00 trigger 基础设施已在 Generation 1 生产中验证；Generation 2 的第一次 scheduled end-to-end acceptance 仍待本次 activation 后的 08:00 实际运行。该链路不需要 Codex、浏览器或终端保持打开。

v0.4.1.2 增加运行时稳定性保护：`scripts/run_daily_digest.sh` 会在任务启动后自动使用 `caffeinate -dimsu` 持有防睡眠 assertion，直到 generation、Obsidian iCloud 同步和 Bark 推送全部执行完毕。任务结束后 `caffeinate` 会随脚本自动退出，不会常驻。
...
脚本会把 `task`、generation entry、`publish_mobile_digest.py` 和 `send_bark_notification.py` 的 start/end、exit code 与耗时写入 launchd 现有 stdout/stderr 日志，便于区分任务晚启动、网络请求变慢和 Bark 单独失败。
