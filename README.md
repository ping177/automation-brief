...
`market_brief` 模式是 v0.5-alpha 的市场投研晨报骨架，必须显式通过配置触发，不改变每日自动运行的默认 digest 链路。输出文件为：

```text
output/market-brief-YYYY-MM-DD.md
```

当前 `market_brief` 只使用离线 mock/sample 数据，section 结构稳定，但不接 AKShare、TuShare 或真实行情，不计算相对强弱，不输出交易动作。持仓观察来自 `config/holdings.json`；如果真实文件不存在，会回退读取 `config/holdings.example.json`。真实 `config/holdings.json` 已加入 `.gitignore`，不要提交成本、仓位、市值、亏损金额或其他敏感持仓信息。

`market_brief` 输出结构：

- 昨日市场环境
- 市场主线：1 日强势、5 日持续强势、20 日趋势主线
- 产业催化
- 我的持仓观察
- 明日观察信号
- 纪律提醒
