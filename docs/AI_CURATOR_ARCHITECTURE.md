...
The formal offline replay of `/private/tmp/automation-brief-live-candidates-20260812T081815290841Z.json` has 159 candidates, 25 capped summaries, and 134 unchanged summaries. With the exact current DeepSeek serializer, the projected request measures `127574` bytes and the provider body measures `138631` bytes; `transport_calls=0`.

The first real RSS + DeepSeek shadow used this same explicit `phase4_live` input and passed the input/body/transport boundary, but failed closed during local response validation because the model repeated an `article_id` in `rejected_article_ids`. The validator requires rejected IDs to be known and globally unique, selected evidence and rejected IDs to be disjoint, per-event evidence IDs to be unique while allowing evidence reuse across events, and event IDs to be unique. The system prompt now states those existing collection invariants and the one-rejection/one-best-reason rule; it does not deduplicate invalid model output or change the validator, schema, limits, retry, or finish-reason behavior.
...
v0.6.1 Phase 1 documentation and feed-language normalization / candidate contract wiring are complete. v0.6.2 Phase 2 provides the adapter and artifact foundation, Phase 3A freezes the DeepSeek request/preflight boundary, Phase 3B completes the offline fixture safety preparation, prompt contract alignment, and successful fixture-only real-provider gate, and Phase 4B freezes the live shadow projection, hard-limit boundary, and first real-output collection-invariant prompt alignment. The overall v0.6.2 evaluation remains incomplete. The next boundary is a separately authorized repeat real DeepSeek shadow evaluation using the explicit `phase4_live` mode; it must remain shadow-only and must not replace daily digest, `market_brief`, or production automation.
