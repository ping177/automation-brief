...
### v0.5.3-alpha news filtering quality tuning

- 收紧 `market_brief` 的重要市场事件筛选，减少泛 IPO、泛估值、泛 AI 商业新闻误入“重要市场事件”。
- 改善观察理由，避免所有新闻都使用同一类模板化解释。
- 合并或规范主题线索，避免 `AI` / `人工智能` 等同义主题重复展示。
- 提高风险/反证栏目的相关性，避免弱相关新闻或普通监管背景误入。
- 提升今日观察清单质量，只保留明确后续节点和可验证变量。
- 提高 holdings 相关新闻匹配精度，优先匹配高价值、直接相关线索，继续避免宽泛标签误挂。
...
### Later: v0.5-beta real A-share market data

- v0.5-beta 再接真实 A 股市场数据；接入前不计算真实板块强度、个股相对强弱或产业催化强度。
- v0.5-beta 应排在 v0.5.3-alpha news filtering quality tuning 之后，避免在新闻质量仍偏泛时叠加行情复杂度。
