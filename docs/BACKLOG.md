...
- Phase 4B 已冻结显式 `phase4_live` provider-facing projection：summary cap=`500`、candidate limit=`200`、provider body limit=`200000`；连续两次真实 DeepSeek shadow 已完成输入/transport 验证但因 duplicate rejected article ID 被 validator 拒绝。现在 live 模式采用 selected-only semantics：rejection enumeration 不再收集，provider boundary 将 rejection 字段 canonicalize 为 `[]`，selected events 仍严格验证；后续 real shadow 仍需单独授权，不能自动切换生产路径。
