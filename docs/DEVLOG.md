...
- focused review Findings 1–4 已以最小修复关闭：RSS 只对 timeout、transient transport、HTTP 429/5xx 重试，普通 4xx 与 deterministic parse/input failure 不重试；`HTTPError` 在 `URLError/OSError` 前分类，exhausted HTTP/parse source failure 保持 frozen `source_fetch_failed` taxonomy。source diagnostics 仅经既有 artifact diagnostic seam 保存 source ref、status、attempt、bounded duration、可选 HTTP status/failure code，不保存 headers、raw response 或 exception/request。
- 正式 runtime builder 现在会在 RSS collection 前实际初始化 pinned `multilingual-e5-small` immutable revision，强制 canonical cache 与 `local_files_only=True`，并让 clustering 复用同一 embedder 实例；wrong-but-nonempty cache 或 snapshot 不可加载会以明确 local model/cache configuration failure fail closed。
- 将 selector/classifier/writer evaluator 中重复的 private DeepSeek wiring 收敛到正式窄 adapter/factory；evaluator 保留兼容入口。新增薄 `scripts/run_generation_2_shadow.py`，只做 explicit real-provider opt-in、report slot、formal runtime invocation 与 safe outcome/artifact output。
- 离线 acceptance 使用 fake fetcher/embedder/gateway 完整运行，验证 invalid slot、missing key/cache fail closed、Gen1 semantic fallback/delivery import isolation、只写 `runs/event-driven-morning-brief/<run_id>/` 且不写 `reports/`。本次未联网、未读取 secret、未调用真实 RSS/DeepSeek/Bark/Obsidian，未修改 main production dispatch、schedule、LaunchAgent、Gen1 或三份 frozen contracts；真实 manual run 等用户审核后在 Terminal 执行。
- focused runtime/evaluator smoke、v1.7 canonical orchestrator snapshot regression、全部 29 个 `tests/offline_*.py`、Python `compileall`、Project-State Push Gate 16/16、shell syntax 与 `git diff --check` 均 PASS。
- 首次真实 Gen2 run 暴露 input population failure：5 个 legacy 新华网 snapshots 与 GitHub Trending Python Daily 共 1521 条 entry 均无 source-provided timestamp，在 nullable admission 下进入 Article pool。Gen2 ingest 现在 collector checkpoint 后、formal normalization 前按 source snapshot 做 freshness qualification；存在有效 entries 但全部无可解析 publication timestamp 的 snapshot 整体排除，只保存 hashed source ref、reason/status 和 bounded excluded count。不用 collected time/URL 猜测，不改 frozen nullable Article contract，不触及 Gen1；Investing.com timezone-less timestamp 仍按既有 normalizer failure 处理。
- 首次真实 run 的3个 production hard negatives 证明纯 threshold correction 不安全：官员/地震/军事部署误合并相似度为 `0.921938 / 0.918545 / 0.915282`，但真实国防部记者会与 CrowdStrike 正例仅为 `0.920543 / 0.920444`。正式 corrective replacement 保留 frozen model/revision、projection、`0.91` base floor 与 connected components，将 algorithm 版本更新为 `identity-guarded-connected-components-v2`、edge policy 为 `semantic-title-anchor-v1`；`>=0.925` 直接接受，ambiguity band 只接受 normalized titles 共享 4-character Unicode alphanumeric span 的 edge。
- Orchestrator 通过现有 bounded diagnostic seam 只持久化 clustering model/revision、projection、algorithm/edge-policy version 和三个 threshold/span 配置值；不持久化 pairwise similarity matrix。v1.7 snapshot 的 Brief/Event/Article/failure 语义不变，仅在 expected artifact inventory 中增加该固定 diagnostic ref。
- 新增独立 `event_clustering_v1_8_corrective.json`，保留原 v1.3 fixture 不改写。pinned local-model focused acceptance 中，原 v1.3 production memberships 仍为 `8/8 exact`；新增 3 hard negatives 全部拆分、2 positive counterexamples 全部聚合，corrective memberships `8/8 exact`，两份 evaluator deterministic repeat 均 PASS。
...
- Writer 只写 `EventWriting` 的 `title_zh`、`summary_zh`、`why_it_matters_zh`，使用完整 Article provenance 与 canonical membership 顺序；不携带 selection、importance、score、legacy、market/holdings 或 clustering diagnostics side channels，summary 不做 Selector-style 截断。
- Response 采用 strict exact-key `writings` shape；unknown/duplicate/missing/malformed/extra response、中文 gate、Article lookup、gateway、transport 与 parse failure 均按 event-local `ItemFailure` 处理，成功 sibling 保留，all-fail 为 `failed`，mixed 为 `partial`，empty input 不调用 gateway。失败 Event 不输出 raw/legacy/placeholder/backfill 内容。
- Writer smoke、Classifier smoke、全部 offline smoke、Python compile/compileall、Project-State gate 与 `git diff --check` 通过；未调用真实 DeepSeek，未修改 frozen contracts、canonical domain、selector、gateway、`main.py`、renderer、artifacts、orchestrator、production routing 或 dependencies。下一步为 Slice 3 classifier → writer continuation regression。
