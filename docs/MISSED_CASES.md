...
## 2026-07-03 v0.5-beta.3 新闻事件排序、合并与 holdings 置信度

- 日期：2026-07-03
- 案例标题：证监会再融资 / 定增政策被普通 IPO 新闻压过，同主题政策重复展示，弱相关海外风电变量挂到持仓下时语气过强
- 原始现象：
  1. “中国证监会就完善上市公司再融资规则公开征求意见”和“中国证监会拟建立上市公司再融资定增储架发行制度”属于同一政策事件，但在重要新闻中分开展示。
  2. 证监会再融资 / 定增政策对 A 股资金供给和上市公司融资节奏更关键，却可能排在普通 IPO / 创业公司融资新闻后。
  3. “400家starup聚集、阿斯利康重押13亿欧元，创新药出海欧洲绕不开这座城｜最前线”更像产业催化 / 医药出海 / 海外投资，不应归为公司融资 / IPO。
  4. “英国改革基建规划审批 力推大规模基建建设”只属于弱相关海外风电 / 基建变量，不应作为金风科技明确相关新闻或高精度线索。
  5. 风险与反证未优先覆盖再融资和定增储架制度的政策变量。
- 漏报 / 误报类型：`event_ranking_gap` / `policy_event_duplicate` / `news_type_misclassification` / `holding_relevance_overstated` / `risk_variable_gap`
- 原因分析：v0.5-beta.2 仍以单条新闻相关度为主，缺少轻量政策事件 key；政策监管词权重不足；公司融资 / IPO 规则对泛资本和海外投资语境仍偏敏感；holdings 相关新闻只有匹配 / 不匹配，没有明确相关、弱相关变量和未解释波动的分层。
- 采取动作：v0.5-beta.3 提高政策监管优先级；为证监会再融资 / 定增 / 储架发行 / 征求意见建立轻量事件合并；收紧 IPO / 融资分类；将创新药出海 / 海外投资归入产业催化；holdings 相关新闻增加明确相关新闻 / 弱相关变量分层；风险与反证补政策变量；补充离线 smoke 回归。
- 回归状态：已扩展 `tests/offline_market_news_smoke.py` 和 `tests/offline_market_brief_smoke.py` 覆盖，并已纳入 v0.5-beta.3 验证范围。

## 2026-07-03 v0.5-beta.3.1 真实样例 hotfix 回归

- 日期：2026-07-03
- 案例标题：再融资 / 定增政策仍未排第一，泛监管误分类，低相关主题和风险变量表达不足
- 原始现象：
  1. “证监会完善上市公司再融资规则，并拟建立定增储架发行制度”仍排在普通 IPO / 创业融资新闻后。
  2. “阿斯利康重押13亿欧元，创新药出海欧洲…”因泛“监管”线索误归为政策监管。
  3. 今日主线已提示“暂未提取到足够明确的产业主线”，但又展示 relevance 约 59 的“云南巧家夏季草莓‘锁鲜’出海”。
  4. 观察理由出现 `出海、出海`、`监管、监管` 等重复命中词。
  5. 风险与反证没有优先抓住再融资 / 定增储架发行制度变量。
  6. “国泰海通预计上半年归母净利润超200亿元”排序低于普通创业融资 / IPO。
- 漏报 / 误报类型：`event_ranking_gap` / `policy_false_positive` / `weak_theme_false_positive` / `reason_keyword_duplicate` / `risk_variable_gap` / `earnings_ranking_gap`
- 原因分析：v0.5-beta.3 的政策权重仍不足以稳定压过高关键词密度 IPO；政策分类对“监管”泛词和否定语境不够严格；今日主线渲染没有按候选相关度设置显示阈值；原因模板拼接未跨词组去重；大型券商业绩预告缺少窄范围优先级。
- 采取动作：v0.5-beta.3.1 强化 A 股制度变量权重，收紧政策监管正向命中，过滤否定语境中的政策词，观察理由关键词去重，今日主线渲染要求 `relevance >= 70`，提升大型金融 / 券商业绩预告排序，风险与反证明确输出再融资和定增储架发行变量。
- 回归状态：已扩展 `tests/offline_market_news_smoke.py` 和 `tests/offline_market_brief_smoke.py` 覆盖；后续真实样例继续观察。
